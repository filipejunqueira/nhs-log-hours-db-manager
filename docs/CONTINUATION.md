# Session Continuation — Website Scaffold
...
> Read docs/WEBSITE_PLAN.md and docs/BUILD_NOTES.md in full, then
> engine_v2/web_data.json and scripts/regen.sh. BUILD_NOTES corrects the plan —
> follow it where they conflict, especially Tailwind v4 (not the v3 config-file
> layout in §6/§8). Then propose a plan to scaffold website/ only: Vite vue-ts,
> Tailwind v4 wiring (@tailwindcss/vite + @import "tailwindcss";), base set to
> /nhs-hour-log/, and stubs for src/types/web-data.ts and src/lib/format.ts. Do
> not build components yet and do not modify engine_v2/. Current data is 12619
> min / 210.32 h (the 11919 figure in the plan is stale); when verifying later,
> check integrity flags are all true and bands/classes sum to the total, not a
> fixed number. Show me the plan before executing.

---

## What Was Done

1. **All four files were read in full**: `docs/WEBSITE_PLAN.md`,
   `docs/BUILD_NOTES.md`, `engine_v2/web_data.json`, `scripts/regen.sh`.
2. **A scaffold plan was written** (see below). It was shown to the user but
   has not been executed yet — no files have been created or modified.

---

## The Plan: Scaffold `website/` — Vite + Vue 3 + TS + Tailwind v4
...
Creates `website/` with `package.json`, `vite.config.ts`, `tsconfig*.json`,
...
Full TypeScript types matching the real `web_data.json` (schema 1.1.0). Types
were verified against the actual emitted JSON — no discrepancies found vs the
§2 sketch.
...
- Strip Vite's default `HelloWorld.vue` and counter example from `App.vue`.
  Replace with a minimal `<h1>NHS Hours Dashboard</h1>` placeholder.
...
1. `npm run dev` serves at `http://localhost:5173/nhs-hour-log/` and shows the
   placeholder heading.
...
- **Read first**: `docs/WEBSITE_PLAN.md` and `docs/BUILD_NOTES.md`. BUILD_NOTES
  wins where they conflict.
- **Do NOT modify `engine_v2/`** — it is locked (also enforced in
  `.claude/settings.json`).
- **Tailwind v4**: no config files, use `@tailwindcss/vite` plugin.
- **The 11919 figure in WEBSITE_PLAN.md §10.6 is stale.** Current data is
  12619 min / 210.32 h. When verifying, check that integrity flags are all
  `true` and that bands/classes each sum to `total_minutes` — don't hardcode
  a specific number.
- **Front end does zero calculation** except minutes ÷ 60 for display.
- **No money, ever.**
- **Schema gate ≥ 1.1.0** (not just "starts with 1.").

---

## Instruction for Next Session

Read this file (`docs/CONTINUATION.md`), then `docs/WEBSITE_PLAN.md` and
`docs/BUILD_NOTES.md`. Execute the plan above — scaffold `website/`, wire
Tailwind v4, create type stubs and format helpers, verify. Do not build
components yet.
